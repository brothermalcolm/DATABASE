DROP PROCEDURE IF EXISTS sp_process_basic_station;

DELIMITER //
CREATE PROCEDURE sp_process_basic_station(station_id int, d date)
...
proc:BEGIN
    ...
    update tbl_staging_basic_station set Tm = str_to_date(obsTm, '%Y-%m-%d %H:%i:%s');
    
    #check if zero rows imported
    select count(1) from tbl_staging_basic_station into @rowcount;
	if @rowcount = 0 then
    ...
    select count(distinct(date(Tm))) from tbl_staging_basic_station into @datecount;
    if @datecount != 1 then
    ...
    select distinct(date(Tm)) from tbl_staging_basic_station into @today;
    if @today != d then
    ...
    insert into tbl_station_spurious_data select station_id, d, AvgGSi00, 'Irradiance measure too high'
    from tbl_staging_basic_station where AvgGSi00 > 1000;
    insert into tbl_station_spurious_data select station_id, d, AvgGsi00, 'Irradiance measure too low'
    from tbl_staging_basic_station where AvgGSi00 < -10;
    ##################################################################################################
    
    #populate tbl_calendar from tbl_calendar_time
    truncate tbl_calendar;
    #select distinct(date(Tm)) into @today from tbl_staging_basic_station;
    ...
    (select Tm from tbl_staging_basic_station);
    
    #set negative values of irradiance during the night to zero
    update tbl_staging_basic_station set AvgGSi00 = 0 where AvgGSi00 < 0;
    
    #flag duplicates
    insert into tbl_station_duplicate_data
    select station_id, Tm, count(1) as cnt from tbl_staging_basic_station group by tm having cnt > 1;
    ...
    insert into tbl_staging_basic_station(station_id, Tm, AvgGSi00, missing)
    ...
    call sp_populate_basic_station(station_id);
